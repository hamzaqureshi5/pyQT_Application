...
#  apply_function(df,'ICCID','ICCID',apply_luhn_check)
  apply_function(df,'ICCID','ICCID',copy_function)
...
def Encoding(df):
  ...
  if 'PIN1' in df.columns: df["PIN1"] = df["PIN1"].apply(enc_pin_func).copy(deep=False)
  if 'PUK1' in df.columns: df["PUK1"] = df["PUK1"].apply(integer_2_ascii).copy(deep=False)
  if 'PIN2' in df.columns: df["PIN2"] = df["PIN2"].apply(enc_pin_func).copy(deep=False)
...
def Decoding(df):
  if 'ICCID' in df.columns: df["ICCID"] = df["ICCID"].apply(dec_iccid).copy(deep=False)
  if 'IMSI' in df.columns: df["IMSI"] = df["IMSI"].apply(dec_imsi).copy(deep=False)
  if 'PIN1' in df.columns: df["PIN1"] = df["PIN1"].apply(dec_pin_func).copy(deep=False)
  if 'PUK1' in df.columns: df["PUK1"] = df["PUK1"].apply(ascii_2_integer).copy(deep=False)
  if 'PIN2' in df.columns: df["PIN2"] = df["PIN2"].apply(dec_pin_func).copy(deep=False)
  if 'PUK2' in df.columns: df["PUK2"] = df["PUK2"].apply(ascii_2_integer).copy(deep=False)
  if 'ADM1' in df.columns: df["ADM1"] = df["ADM1"].apply(ascii_2_integer).copy(deep=False)
  return df
...
def _DATA_EXTRACTOR(input_dict:dict,df_input,clip:bool,encoding:bool,caption:str):
  laser_h,laser_d_h,laser_unique,laser_c,laser_l,laser_r =get_dic_parameter(input_dict)
  df20 = df_input.copy(deep=False)
  if encoding is True: 
    emcoded_df = Decoding(df20)
  else: 
    emcoded_df = df20.copy(deep=False)
#  print(laser_l)
#  print(laser_r)
...
#df=pd.read_csv("Input Files/demo2.csv",dtype = str)
#print(df)
#print(_DATA_EXTRACTOR(input_elect_params,df,True,True,""))

print(dec_imsi("084901408088707482"))